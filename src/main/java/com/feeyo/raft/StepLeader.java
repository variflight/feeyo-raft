...
public class StepLeader {
    ...
    public static void step(Raft r, Message msg) throws RaftException {
		...
		switch(msgType) {
		case MsgBeat:
			r.bcastHeartbeat(); // 广播 HeartBeat 消息
			return;
		case MsgCheckQuorum:
			...
			if( !r.checkQuorumActive() ) {
				...
				r.becomeFollower(r.term, Const.None); // 变成follower状态
			}
			return;				
		case MsgPropose:
			...
			if (r.quorum() > 1) {
				...
				case LeaseBased:
					...
					if (msg.getFrom() == Const.None || msg.getFrom() == r.id) { // from local member
						//
						// from local message (TODO: Execute readIndex callback)
						String rctx = msg.getEntries(0).getData().toStringUtf8();
						r.raftNodeListener.onReadIndex(rctx, r.raftLog.getCommitted());
		...
		case MsgAppendResponse:
			...
			pr.setRecentActive(true);	// 更新该节点当前是活跃的
			...
			if (msg.getReject()) {
				...
				// RejectHint 带来的是拒绝该app请求的节点，其最大日志的索引(TODO: 尝试回退关于该节点的Match、Next索引)
				if (pr.maybeDecrTo(msg.getIndex(), msg.getRejectHint())) {
					...
					r.sendAppend(msg.getFrom()); // 再次发送append消息
				}
				
			} else {
				boolean oldPaused = pr.isPaused(); // 通过该append请求
				if (pr.maybeUpdate(msg.getIndex())) { // 如果该节点的索引发生了更新, 调整该 Progress 的 matched & nextIdx
					
					switch(pr.getState()) {
					case Probe:
						pr.becomeReplicate(); // 如果当前该节点在探测状态，切换到可以接收副本状态
						break;
					case Snapshot:
					...
					if (msg.getFrom() == r.leadTransferee && pr.getMatched() == r.raftLog.lastIndex()) {
						...
						r.sendTimeoutNow(msg.getFrom()); // 要迁移过去的新leader，其日志已经追上了旧的leader
					}
				}
			}
			break;
		...
		case MsgHeartbeatResponse:
			...
			pr.setRecentActive(true); // 该节点当前处于活跃状态
			pr.resume(); // 这里调用resume是因为当前可能处于probe状态，而这个状态在两个HeartBeat消息的间隔期只能收一条同步日志消息，因此在收到HeartBeat消息时就停止pause标记
			//
			...
			if (Util.isNotEmpty(rss)) {
				...
				for (ReadIndexStatus rs : rss) {
					...
					if (req.getFrom() == Const.None || req.getFrom() == r.id) {
						...
						r.raftNodeListener.onReadIndex(rctx, rs.index);
					} else {
						// 否则就是来自外部，需要应答
						Message newMsg = Message.newBuilder() //
								.setTo(req.getFrom()) //
		...
		case MsgTransferLeader:
			...
			if (lastLeadTransferee != Const.None) {
				// 判断是否已经有相同节点的leader转让流程在进行中(TODO:如果是，直接返回)
