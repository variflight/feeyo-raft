...
public class RaftServerDefaultImpl extends RaftServer {
	...
	//
	public RaftServerDefaultImpl(RaftConfig raftConfig, StateMachineAdapter stateMachine) {
    ...
    protected void onNewReady(Ready ready) throws RaftException {
		...
		final SnapshotMetadata snapshotMetadata = ready.unstableSnapshotMeta;
		if (snapshotMetadata != null && snapshotMetadata.getIndex() > 0) {
			...
			applySnapshotToStateMachine(snapshotMetadata);
