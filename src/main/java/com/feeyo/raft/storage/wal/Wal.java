...
public class Wal {
	...
	public ReadAllLogs readAll(long snapshotIndex) throws Throwable {
		...
		try {
			...
			for (int i = 0; i < fileNames.size(); i++) {
				...
				AbstractLogFile.LogMetadata logMeta = logFile.openAtIndex( snapshotIndex );
				if ( logMeta.getStart() != null ) // WalSnap
					start = logMeta.getStart();
				//
				if ( Util.isNotEmpty( logMeta.getEntries() ) )  // Entries
					entries.addAll( logMeta.getEntries() );
				//
				if ( logMeta.getHs() != null ) // HardState
					lastHs = logMeta.getHs();
				//
				if ( !isWritable )
					logFile.close();
				//
			...
			//
			return new ReadAllLogs(entries, lastHs);
	...
	public void save(List<Entry> entries, HardState hs) throws RaftException {
		...
		try {
			...
			// 这些数据是否需要落盘
			boolean mustSync = Util.isMustSync(hs, this.state, Util.isNotEmpty(entries) ? entries.size() : 0);
	...
	//
	//
	public static class ReadAllLogs {
		private List<Entry> entries;
		private HardState hs;
	
		public ReadAllLogs(List<Entry> entries, HardState hs) {
