 ...
 * @author zhuam
 */
public class LogFileMmapImpl extends AbstractLogFile  {
	
	private static Logger LOGGER = LoggerFactory.getLogger( LogFileMmapImpl.class );
    ...
    // 是否已预热
    protected boolean isWarmed = false;
    
	@SuppressWarnings("resource")
	public LogFileMmapImpl(String path, long firstLogIndex, long lastLogIndex, 
		...
		//
		this.fileName = AbstractLogFile.toFileName(firstLogIndex, lastLogIndex);
	...
	public LogMetadata openAtIndex(long index) throws RaftException {
		...
		try {
            	...
            	if ( startMagic == START_MAGIC && ((fileSize - wrotePos.get() - START_LEN - dataLength - END_LEN) >= 0) )  {
                	...
                	if ( endMagic == END_MAGIC ) {
                		...
                		if ( dataType == AbstractLogFile.DataType.ENTRY ) {
        					...
        					if (entry == null) 
        						throw new Errors.RaftException("load file err, fileName=" + fileName);
        					//
			...
			throw new Errors.RaftException("load err:" + fileName + ", size=" + fileSize + ", wrote=" + wrotePos, e);
		} 
		//
	...
	public void truncate(long index) {
		try {
			...
			if (isClosed.get()) {
				...
				this.lastLogIndex = 0;
				//
				String newFileName = AbstractLogFile.toFileName(firstLogIndex, lastLogIndex);
				File newFile = new File(this.path, newFileName);
				// rename with Google Guava
	...
	public String toString() {
		...
		sBuffer.append("[ ").append(fileName);
		sBuffer.append(", wrotePos=").append(wrotePos);
		sBuffer.append(", committedPos=").append(committedPos);
		sBuffer.append(" ]");
