...
public class FileBuffer {
	...
	//
	private SegmentFile findSegmentFileByPos(long position) {
	...
	//
	public long writeByteArray(byte[] data) throws IOException {
		int waitToWroteSize = data.length;
		ensureMaxCapacity( waitToWroteSize );
		//
		SegmentFile lastFile = getLastSegmentFile( waitToWroteSize );
		lastWrotePos = lastFile.writeByteArray(data);
		return lastWrotePos;
	}

	public byte[] readByteArray(long position, int length) {
		SegmentFile file = findSegmentFileByPos(position);
	...
	static class SegmentFile {
		...
		//
		// 避免 readByteBuffer 的线程安全问题，因为 slice 里面存在修改 position 的问题
		...
		//
		public long writeByteArray(byte[] data) {
			synchronized ( _lock ) {
		...
		public byte[] readByteArray(long position, int length) {
			...
			for (int i = 0; i < length; i++) 
				data[i] = mappedByteBuffer.get(pos + i);
			return data;
		}
