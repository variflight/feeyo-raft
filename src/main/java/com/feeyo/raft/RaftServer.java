 ...
 * @see https://www.codedump.info/post/20210515-raft/
 * 
 * @author xuwenfeng
 * @author zhuam
 */
...
public abstract class RaftServer extends RaftNodeAdapter {
	...
	protected volatile HardState prevHs;		// 硬状态，存储了term commit and vote, 需要持久化
	//
	// WAL & Snapshot
	protected Wal wal;
	...
	//
	public RaftServer(RaftConfig raftConfig, StateMachineAdapter stateMachine) {
		this.raftConfig = raftConfig;
		this.storageDir = raftConfig.getCfg().getStorageDir() + File.separator;
		this.peerSet = raftConfig.getPeerSet();
		...
		int coreThreads = Math.max(this.raftConfig.getTpCoreThreads() / 2, 4);
		int maxThreads = Math.max(this.raftConfig.getTpMaxThreads(), coreThreads);
		int queueCapacity = Math.min(this.raftConfig.getTpQueueCapacity(), 1500);
		//
	...
	public void start(int reactorSize) throws Throwable {
		...
		try {
			...
			this.wal = new Wal(storageDir, raftConfig.getCfg().getMaxLogFileSize(), raftConfig.getCfg().isSyncLog());
			Pair<List<Entry>, HardState> pair = wal.readAll( snapshotMeta == null ? -1 : snapshotMeta.getIndex() );
			List<Entry> ents = pair.first;
			HardState lastHs = pair.second;
			LOGGER.info("self={}, replay wal, hs={}, ents={}", getId(), Util.toStr(lastHs), Util.toStr(ents));

			//
			// 3、Rebuilding storage
			storage.applySnapshotMetadata(snapshotMeta);
			storage.setHardState(lastHs);
			storage.append(ents);
			LOGGER.info("self={}, replay storage, firstIdx={}, lastIdx={}", getId(), storage.firstIndex(), storage.lastIndex());
			//
			// 4、Initialize raft
			this.raft = new Raft(raftConfig.getCfg(), storage, this);
			...
			if ( lastIndex == 0 ) {
				...
				for(Peer peer: raftConfig.getPeerSet().values()) {
			...
			this.transportServer.start(raftConfig.getLocal().getPort(), reactorSize);
			...
			System.out.println("raft startup, port=" + raftConfig.getLocal().getPort() );
			//
		} catch (Throwable e) {
			...
			public void run() {
				...
				while (running.get()) {
					try {
						...
						//
						// Save raft entries to wal, then publish over commit channel
	...
	protected List<Entry> entriesToApply(List<Entry> ents) throws RaftException {
		...
		if (firstIdx > raft.getRaftLog().getApplied() + 1)
            throw new Errors.RaftException(String.format("first index of committed entry[%s] should <= progress.appliedIndex[%s]+1", firstIdx, raft.getRaftLog().getApplied()));
        // E.g:
        ...
        for (Entry ent: entries) {
            if (ent.getData() == null || ent.getData().isEmpty()) {
	            ...
	            if ( entryType == EntryType.EntryNormal ) {
	            	...
	            	//
	            	// TODO: Q1:状态机内部错误，此处如果捕获了，后面不进行 setApplied，大概率会造成阻塞！！！
					...
					try {
						...
						if (isChanged) {
							if (transportClient != null)
								transportClient.reload( peerSet ); // transportClient 热更新
							//
							raft.applyConfChange(cc); // 通知Raft内部 progress 信息改变
							stateMachine.applyMemberChange(peerSet, ent.getIndex()); // 通知状态机，成员改变
			...
			raft.getRaftLog().setApplied(newAppliedIndex);	// TODO: 修改 applied
            //
			if (LOGGER.isDebugEnabled())
				LOGGER.debug("after commit, applied={}", newAppliedIndex);
        }
    }
    ...
    protected void maybeTriggerLocalSnapshot() throws RaftException {
        ...
        final long snapCount = raftConfig.getCfg().getSnapCount();
        ...
        if ( interval < raftConfig.getCfg().getSnapInterval() )
        	return;
        //
        // 是否正在创建本地快照
	...
	//
	public long getId() {
		return raftConfig.getLocal().getId();
	}
