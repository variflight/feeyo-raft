...
public class RaftGroupServer {
	...
	//
	protected RaftGroupConfig raftGroupCfg;
	...
	//
	//
	public RaftGroupServer(RaftGroupConfig raftGroupCfg, RaftGroupStateMachineAdapter stateMachine) {
		//
		this.raftGroupCfg = raftGroupCfg;
		this.peerSet = raftGroupCfg.getPeerSet();
		this.stateMachine = stateMachine;
		//
		//  Build groups
		final Config cfg = raftGroupCfg.getCc();
		for(Region region: raftGroupCfg.getRegions())
			rfGroups.put(region.getId(), new RaftGroup(this, region, cfg) );
		//
		// Asynchronous thread pool  dispatch & notify
		int coreThreads = Math.max(raftGroupCfg.getTpCoreThreads() / 3, 4);
		int maxThreads = Math.max(raftGroupCfg.getTpMaxThreads(), coreThreads);
		int queueCapacity = Math.min(raftGroupCfg.getTpQueueCapacity(), 1500);
		//
	...
	public void start(int reactorSize) throws Throwable {
		...
		try {
			...
			this.transportServer.start( raftGroupCfg.getLocal().getPort(), reactorSize);
			//
			...
			//
			System.out.println("raft-group startup, port=" + raftGroupCfg.getLocal().getPort() );
			//
		} catch (Throwable e) {
	...
	//
	public long getId() {
		return raftGroupCfg.getLocal().getId();
	}
