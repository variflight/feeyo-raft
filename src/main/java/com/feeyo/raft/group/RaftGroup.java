    ...
    public void start() throws Throwable {
		...
		if (lastIndex == 0) {
            ...
            for(long pid: cfg.getVoters()) {
				...
				entries.add(entry);
                raft.addVoterOrLearner(peer.getId(), peer.isLearner());	 // add voter
            }
            for(long pid: cfg.getLearners()) {
                Peer peer = raftGroupSrv.getPeerSet().get( pid );
                Entry entry = newConfChange(ConfChangeType.AddNode, peer);
				entries.add(entry);
                raft.addVoterOrLearner(peer.getId(), peer.isLearner());	 // add learner
            }
    ...
    private void onNewReady(final Ready ready) throws RaftException {
		...
		if (isLeader) {
            ...
            this.raftGroupSrv.getDispatchTpExecutor().execute( new Runnable(){
                ...
                public void run() {
                    try {
                        // TODO：Apply snapshot 这个地方会造成阻塞，很大可能会触发新的选举, 后续优化
    ...
    protected List<Entry> entriesToApply(List<Entry> ents) throws RaftException {
        ...
        if( firstIdx > raft.getRaftLog().getApplied() + 1)
            throw new Errors.RaftException(String.format("first index of committed entry[%s] should <= progress.appliedIndex[%s]+1", 
            		firstIdx, raft.getRaftLog().getApplied()));
        // E.g:
