...
/**
 * -------------------------------------------------------------------------------------------------------------------------
 * DIFF: corePoolSize 、 maximumPoolSize
 * 
 * 1、新任务， 池子内线程小于  corePoolSize， 则创建新的线程执行 ， 即使其他工作线程空闲,也依然创建
 * 2、新任务， 池子内线程 超过 corePoolSize 但小于 maximumPoolSize， 将任务添加至 queue, 仅当队列已满时才会创建新线程
 * 3、达到  maximumPoolSize， 拒绝
 * 
 * 
 * 
 * 代码和思路主要来自于： 
 ...
 * 执行策略： 优先offer到queue，queue满后再扩充线程到maxThread，如果已经到了maxThread就reject 
 * 比较适合于CPU密集型应用（比如runnable内部执行的操作都在JVM内部，memory copy, or compute等等）
 * 
 * StandardThreadExecutor execute 
 * 执行策略：优先扩充线程到maxThread，再offer到queue，如果满了就reject 比较适合于业务处理需要远程资源的场景
 * 
 * @author maijunsheng
 * @author zhuam
 * 
...
class ExecutorQueue extends LinkedTransferQueue<Runnable> {
	...
	public boolean force(Runnable o) {
		if (threadPoolExecutor.isShutdown()) {
			throw new RejectedExecutionException("ThreadPoolExecutor not running, can't force a command into the queue");
		}
